...
<div class="content content-fixed ">
   <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0  ">
        ...
        @if($vendor_msa->status == 'start' || $vendor_msa->status == 'negotiation' || $vendor_msa->status == 'workflow')
            ...
            <div class="position-relative">
                ...
                <h4 class="mg-b-0 tx-spacing--1">Workflow: Vendor MSA</h4>
                    ...
                    @php 
                        ...
                        $flow_teamno= (int)$vendor_flow_teamno;
                        $ven_team = explode(" ", $vendor_team);
                        $totalmem = count($ven_team);
                       $vendormsaid = Crypt::encrypt($vendor_msa->id); 
                    @endphp

                @if($totalmem > $flow_teamno)
                <div class="create-new-btn custom-btn m-0" style="position:absolute;right: 0%;top: -2px;">
                    <form method="POST" action="{{ route('user-vendor-msa-sent-next-teammember') }}">
                        @csrf
                            <input type="hidden" name="vendormsaid" value={{$vendor_msa->id}}> 
                            <button class="req_for_sign_btn" type="submit" >Send To Next Team Member</button>
                     </form>
                </div>
                @endif
                ...
                <div class="create-new-btn custom-btn m-0" style="position:absolute;right: 427px;top: 11px;">
                    ...
                    <a href="{{ URL::asset('/public/'.$vendor_msa->document_original) }}" target="_blank"  class="req_for_sign_btn"  style="padding: 15px 30px">View Document</a>
                   
                </div>
                
                <div class="create-new-btn custom-btn m-0" style="position:absolute;right: 240px;top: 11px;">
                  
                  <!-- https://6478498c91ce81469e93b3c4--gorgeous-haupia-d3c739.netlify.app -->
                  <!-- https://resolutecontracts.netlify.app -->
                  
                    <a href="https://64802b9d957b49395510faf0--eclectic-kashata-047f58.netlify.app/vendor-msa/<?php echo $vendormsaid ?>"  class="req_for_sign_btn" style="padding: 15px 30px">Edit Document</a>
                    
                </div>
                <div class="create-new-btn custom-btn m-0" style="position:absolute;right: 0;top: -2px;">
                      <form method="POST" action="{{ route('user-vendor-msa-request-signature') }}">
                        @csrf
                            <input type="hidden" name="vendormsaid" value={{$vendor_msa->id}}> 
        ...
        @else
            ...
            @if(!empty($vendor_msa->documentId))
                    ...
                    @if($vendor_msa->status == 'request-for-sign')
                    <div class="position-relative">
                        <nav aria-label="breadcrumb">
                           <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                              <li class="breadcrumb-item"><a href="#">Vendor MSA</a></li>
                        ...
                        <h4 class="mg-b-0 tx-spacing--1">Signatureflow: Vendor MSA</h4>
                    
                    </div>
                    <iframe
                        id = "prepare_page"
                        src = "{{$vendor_msa->sendUrl}}"
                        width = "100%"
                        class = "frame"
                        style="height: 80vh;">
                    </iframe>
                    ...
                    @if($vendor_msa->status == 'completed')
                    <div class="position-relative">
                        <nav aria-label="breadcrumb">
                           <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                              <li class="breadcrumb-item"><a href="#">Vendor MSA</a></li>
        ...
        document.addEventListener("adobe_dc_view_sdk.ready", function () {
            ...
            adobeDCView.registerCallback(
                ...
                function (metaData, content, options) {
                    ...
                    $.ajax({
                        url:"{{url('/user/save-workflow/vendor-msa')}}",
                        contentType: false,
                        processData: false,
                        data: formData,
                        type: 'POST',
